...
public class LooniumComponent extends SerializableComponent {
	public static final Identifier ID = botaniaRL("loonium_drop");
	...
	public void readFromNbt(CompoundTag tag, HolderLookup.Provider registryLookup) {
		if (tag.contains(TAG_TO_DROP)) {
			setDrop(ItemStack.parseOptional(registryLookup, tag.getCompoundOrEmpty(TAG_TO_DROP)));
		} else {
	...
	public void writeToNbt(CompoundTag tag, HolderLookup.Provider registryLookup) {
		if (isOverrideDrop()) {
			if (!getDrop().isEmpty()) {
				tag.put(TAG_TO_DROP, getDrop().save(registryLookup));
			}
